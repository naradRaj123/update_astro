...
const StatBox = ({ title, value, icon: IconComponent, bgColor = "bg-red-100", textColor = "text-red-700" }) => (
  <motion.div
    initial={{ opacity: 0, y: 15 }}
    animate={{ opacity: 1, y: 0 }}
    transition={{ duration: 0.4 }}
    className={`p-4 rounded-lg shadow ${bgColor} cursor-pointer`}
  >
...
const ActionButton = ({ label, icon: IconComponent, onClick, variant = "default", className = "", disabled = false }) => (
  <Button
    onClick={onClick}
    variant={variant}
...
const AstrologerDashboard = () => {
  ...
  const [walletInput, setWalletInput] = useState(user?.wallet || 0);
  ...
  const handlePaymentRequest = async (e) => {
    ...
    try {
      ...
      toastr.error("Withdrawal request failed.");
      setOpenModal(false);
    }
  };

  const handleProfileSettingsClick = () => navigate("/astro-update");
      ...
      <motion.div className="container mx-auto" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          <div onClick={() => setOpenModal(true)}>
            <StatBox title="Today's Earnings" value={`₹ ${user.wallet || 0}`} icon={DollarSign} bgColor="bg-green-100" textColor="text-green-700" />
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <Card className="md:col-span-1 shadow-xl rounded-xl">
            ...
            <CardContent className="space-y-4">
              <ActionButton label={isCallEnabled ? "Disable Calls" : "Enable Calls"} icon={Phone} onClick={() => setIsCallEnabled(!isCallEnabled)} variant={isCallEnabled ? "destructive" : "default"} />
              <ActionButton label={isChatEnabled ? "Disable Chat" : "Enable Chat"} icon={MessageSquare} onClick={() => setIsChatEnabled(!isChatEnabled)} variant={isChatEnabled ? "destructive" : "default"} />
              <ActionButton label={isVideoEnabled ? "Disable Video" : "Enable Video Call"} icon={Video} onClick={() => setIsVideoEnabled(!isVideoEnabled)} variant={isVideoEnabled ? "destructive" : "default"} />
      ...
      <Dialog.Root open={openModal} onOpenChange={setOpenModal}>
        <Dialog.Portal>
          ...
          <Dialog.Content className="fixed left-1/2 top-1/2 w-[90%] max-w-sm -translate-x-1/2 -translate-y-1/2 bg-white p-6 rounded-lg shadow-xl z-50">
            <Dialog.Title className="text-lg font-semibold mb-4">Withdraw Earnings</Dialog.Title>
